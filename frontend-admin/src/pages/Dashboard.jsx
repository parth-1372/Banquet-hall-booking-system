        ...
        <AdminLayout title="Dashboard Overview">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
                {statCards.map((stat, i) => (
                    <div
                        ...
                        className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all group cursor-pointer"
                    >
                        ...
                        <div>
                            ...
                            <p className="text-3xl font-black text-slate-800 dark:text-white">{loading ? '...' : stat.value}</p>
                        </div>
                    </div>
                ))}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 bg-white dark:bg-slate-900 p-8 rounded-[40px] border border-slate-100 dark:border-slate-800 shadow-sm transition-colors">
                    <div className="flex justify-between items-center mb-10">
                        <div>
                            ...
                            <p className="text-sm text-slate-400 dark:text-slate-500 font-medium">Weekly performance overview</p>
                        </div>
                        <select className="bg-slate-50 dark:bg-slate-800 border-none rounded-xl text-sm font-bold text-slate-700 dark:text-slate-300 px-4 py-2 outline-none">
                ...
                <div className="bg-white dark:bg-slate-900 p-8 rounded-[40px] border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col transition-colors">
                    <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-8">Recent Bookings</h3>
                    <div className="space-y-6 flex-1 overflow-auto pr-2 no-scrollbar">
                        {loading ? (
                            ...
                            stats.recentBookings.map((booking) => (
                                <Link to="/bookings" key={booking.id} className="flex gap-4 p-4 rounded-2xl hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors border border-transparent hover:border-slate-100 dark:hover:border-slate-800 group">
                                    <div className="w-12 h-12 bg-sky-50 dark:bg-sky-900/20 rounded-xl flex items-center justify-center text-sky-500 font-bold">
                                        {booking.user?.name?.[0]}
                                    </div>
                                    ...
                                    <div className="text-right">
                                        ...
                                        <p className="text-[9px] text-slate-400 font-bold uppercase">{new Date(booking.createdAt).toLocaleDateString()}</p>
                                    </div>
                                </Link>
                            ))
                        ) : (
                    ...
                    <Link
                        to="/bookings"
                        className="mt-8 py-4 bg-slate-50 dark:bg-slate-800 rounded-2xl text-center text-admin-primary font-bold text-sm hover:bg-admin-primary hover:text-white transition-all flex items-center justify-center gap-2 group"
                    >
